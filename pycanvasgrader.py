...
# library
from lib.pyCanvasApi import pyCanvasApi as pCA
from lib.pyCanvasApi import pyCanvasTesting as pCT
...
def close_program(grader: pCA.PyCanvasGrader, restart=False):
    grader.close()
    if restart:
        init_tempdir()
        main()
...
def choose_course(course_list) -> int:
    return utils.choose(
        ...
        formatter=lambda c: '%s (%s)' % (c.get('name'), utils.month_year(c['start_at']))
    ).get('id')
...
def save_state(grader: pCA.PyCanvasGrader, test_skeleton: pCT.TestSkeleton, users: List[pCA.User]):
    ...
    try:
        ...
        print('Make sure the program has permission to read/write in {}'.format(os.path.join(os.environ['INSTALL_DIR'], '.cache')))
...
def load_state(course_id: int, assignment_id: int):
    ...
    with open('.cachefile') as cache_file:
        ...
        test_skeleton = pCT.TestSkeleton.from_json(cache['skeleton'])
        users = [pCA.User.from_json(userdata) for userdata in cache['users']]

        os.chdir(os.environ['INSTALL_DIR'])
        return test_skeleton, users


def grade_all_submissions(test_skeleton: pCT.TestSkeleton, users: List[pCA.User], only_ungraded: bool = False) -> bool:
    if only_ungraded:
...
def submit_all_grades(grader: pCA.PyCanvasGrader, users: list) -> bool:
    modified = False
    user_data = []
    for user in users:
...
def user_menu(grader: pCA.PyCanvasGrader, test_skeleton: pCT.TestSkeleton, user: pCA.User):
    global CURRENTLY_SAVED
    ...
    while True:
        ...
        if choice == possible_opts['log']:
            ...
            user.grade = utils.choose_val(1000, allow_negative=True, allow_zero=True, allow_float=True)
            if user.grade != grade_before:
                CURRENTLY_SAVED = False
            utils.clear_screen()
...
def main_menu(grader: pCA.PyCanvasGrader, test_skeleton: pCT.TestSkeleton, users: list, prefs: dict):
    global CURRENTLY_SAVED
...
def startup(grader: pCA.PyCanvasGrader, prefs: dict) -> (int, int):
    ...
    try:
        ...
        selected_role = utils.Enrollment[utils.choose(
            ['teacher', 'ta'],
...
def grade_assignment(grader: pCA.PyCanvasGrader, prefs: dict):
    session = prefs['session']
    quickstart = prefs['quickstart']

    # Get list of submissions for this assignment
    submission_list = [s for s in grader.submissions() if s.get('workflow_state') != 'unsubmitted']
    ...
    for count, submission in enumerate(submission_list):
        ...
        if submission.get('attachments') is not None:
            utils.print_on_curline('downloading submissions... ({}/{})'.format(count, total))
            if grader.download_submission(submission):
                user_data = grader.user(user_id)
                users.append(pCA.User(user_id, submission['id'], user_data['name'], user_data.get('email'),
                                      submission['score'], submission['grade_matches_current_submission'], submission['attempt']))
            else:
                failed += 1
    utils.print_on_curline('Submissions downloaded. ({} total, {} failed to validate)\n\n'.format(total, failed))
    ...
    if quickstart.get('skeleton'):
        selected_skeleton = pCT.TestSkeleton.parse_skeleton(quickstart.get('skeleton'))

    if selected_skeleton is None:
        skeleton_list = pCT.TestSkeleton.parse_skeletons(os.path.join(os.environ.get('INSTALL_DIR'), 'skeletons'))
